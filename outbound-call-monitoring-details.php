...
<?php ($role > 5) ? print("<script>alert('You are Lost');window.location.href ='index.php';</script>") : '' ?>
...
if ($totalCount == $target) {
    print("<script>alert('Monthly Target Of $target Audit Achieved');window.location.href='outbound-call-monitoring.php'</script>");
}
?>
<?php
$loginid = $_SESSION['loginid'];
...
$resultId = mysqli_query($link, "SELECT audit_report.*,employee_audit_report.agentCallid,employee_audit_report.employeeid,employee_audit_report.employeeName ,
    ...
    JOIN employee_audit_report ON audit_report.agentCallid = employee_audit_report.agentCallid where audit_report.agentCallid = '$agentCallid' AND audit_report.id = '$id'");
                    ...
                    <div class="col-12 col-md-12 col-lg-12">
                        <div class="bg-primary row rounded">
                            <div class="text-center w-100 p-2">
                                ...
                                <h5><b>Outbound Call Monitoring Form</b></h5>
                            </div>
                        </div>
                        <form class="mt-4" action="outbound-call-monitoring-process.php" method="POST">
                ...
                <div class="card">
                    <div class="card-body">
                        <table class="table">
                            ...
                            <?php
                            $allQtag = array();
                            $getQuestions = mysqli_query($link, "SELECT * FROM outbound_question_set");
                            ...
                            if ($queNum > 0):
                                while ($row = mysqli_fetch_assoc($getQuestions)):
                                    ...
                                    <?php
                                    $qTag = $row['qTag'];
                                    $getSubparameter = mysqli_query($link, "SELECT * FROM subParameter_outbound_question_set where qTag=" . "'$qTag'");
...
<?php
$allFtlQ = mysqli_query($link, "SELECT qTag FROM outbound_question_set WHERE isFatal!=''");
$fatalQue = array();
...
<script>
    function getScore(qid) {
        ...
        $.ajax({
            url: "oubound-calculating-score.php",
            type: "POST",
            data: {
                qid: qid,
                qVal: qVal,
