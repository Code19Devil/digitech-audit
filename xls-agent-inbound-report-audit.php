...
// $fSql = "SELECT cName keyValues, IFNULL(qHeader, IF(cName='handlingUser','auditorName',cName)) headerValues FROM (
        //  SELECT COLUMN_NAME cName FROM INFORMATION_SCHEMA.COLUMNS WHERE TABLE_NAME='inbound_call_monitoring' AND TABLE_SCHEMA='ippb_audit') COLS 
        //  LEFT JOIN (select qTag, qHeader FROM inbound_question_set ) COL_HEAD ON cName=qTag";
...
FROM(SELECT * FROM (SELECT COLUMN_NAME cName FROM INFORMATION_SCHEMA.COLUMNS WHERE TABLE_NAME='inbound_call_monitoring' AND TABLE_SCHEMA='ippb_audit') COLS
LEFT JOIN (SELECT subTag, qDescription FROM subParameter_inbound_question_set ) COL_HEAD2 ON cName=COL_HEAD2.subTag
LEFT JOIN (SELECT qTag, qHeader,weightage FROM inbound_question_set ) COL_HEAD ON cName=COL_HEAD.qTag) level2) level3) level4
";
...
$sql = "SELECT $sqlHeader FROM inbound_call_monitoring m 
...
$fname = "ippb_agent_inbound_audit_report.xlsx";
$arr = array();
array_push($arr, $vArr);
