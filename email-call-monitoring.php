...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
  ...
  $sql = "Select * from(Select * from(SELECT * FROM audit_report WHERE  status= 'pending' AND lob = 'email' and disposition is not null ANd is_active =1";
  ...
  $sql = "SELECT * FROM audit_report WHERE status= 'pending' AND assignedTo = '$loginid' AND lob = 'email' AND disposition is not null ANd is_active =1 Order by assignedTime desc  limit 1";
  $result = mysqli_query($link, $sql);
              ...
              <div class="card-body">
                ...
                <div class="card shadow p-4">
                  <div class="row">
                    ...
                    <div class="col-lg-3 col-12 m-auto">
                      <?php $res = mysqli_query($link, "SELECT COUNT(*) AS `count` FROM audit_report WHERE status= 'closed' AND assignedTo = '$loginid' AND lob = 'email'");
                  ...
                  <form action="xls-agent-email-report-audit.php" method="POST">
                    <div class="row">
                    ...
                    <?php if (mysqli_num_rows($result) > 0): ?>
                      <?php while ($row = mysqli_fetch_assoc($result)): ?>
                        ...
                        <tr>
                          ...
                          <td>
                            <a href="email-call-monitoring-details.php?agentCallid=<?php print $row['agentCallid']; ?>&id=<?php print $row['agentCallid']; ?>"
...
<script>
  window.onpopstate = function (event) {
    if (event) {
      window.location.href = '<?php echo $baseURL ?>email-call-monitoring.php';
    }
  };
</script>
</body>
