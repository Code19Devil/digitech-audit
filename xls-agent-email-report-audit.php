...
//  SELECT COLUMN_NAME cName FROM INFORMATION_SCHEMA.COLUMNS WHERE TABLE_NAME='email_call_monitoring' AND TABLE_SCHEMA='ippb_audit') COLS 
//  LEFT JOIN (select qTag, qHeader FROM email_question_set ) COL_HEAD ON cName=qTag";
...
$fSql = "SELECT keyValues,IF(weightage,CONCAT('(',weightage, ') ' ,headerValues),headerValues) headerValues FROM
        ...
        FROM(SELECT * FROM (SELECT COLUMN_NAME cName FROM INFORMATION_SCHEMA.COLUMNS WHERE TABLE_NAME='email_call_monitoring' AND TABLE_SCHEMA='ippb_audit') COLS
        LEFT JOIN (SELECT subTag, qDescription FROM subParameter_email_question_set ) COL_HEAD2 ON cName=COL_HEAD2.subTag
        LEFT JOIN (SELECT qTag, qHeader,weightage FROM email_question_set ) COL_HEAD ON cName=COL_HEAD.qTag) level2) level3) level4";
...
$sql = "SELECT $sqlHeader FROM email_call_monitoring m 
...
$fname = "ippb_agent_email_audit_report.xlsx";
